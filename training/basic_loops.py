import torch
from util.data import create_rdms, rnn_reshaping, rnn_unshaping
...
def rnn_train_loop(path, dataloader, model, loss_fn, optimizer, alpha):
    model.train()
...
def rnn_test_loop(path, dataloader, model, loss_fn):
    model.eval()
    size = len(dataloader.dataset)
    test_loss, correct = 0, 0
